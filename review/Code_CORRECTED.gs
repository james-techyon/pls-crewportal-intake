 ...
 * 
 * 1. EMAIL SENDER: Emails will be sent FROM the Google account that deploys this script.
 *    - If you want emails to come from kyle@prestigelaborsolutions.com, Kyle must deploy this script from his Google account
 *    - Google Apps Script does not allow setting a custom "from" address
 * 
 * 2. DEPLOYMENT VERSION ISSUE FIX:
 *    After copying this code:
 *    a) Save the project (Ctrl+S or Cmd+S)
 *    b) Click "Deploy" > "Manage deployments"
 *    c) Click the pencil icon ✏️ next to your active deployment
 *    d) In "Version", select "New version" from the dropdown
 *    e) Add a description like "Fixed email notifications"
 *    f) Click "Deploy"
 *    
 *    This ensures your new code is actually running, not an old cached version.
 * 
 * 3. TO TEST EMAIL SENDING:
 *    - Run the testEmailSending() function from the Apps Script editor
...
Prestige Labor Solutions Team
  ...
  try {
    ...
    console.log('Applicant confirmation sent to:', formData.email);
  } catch (error) {
...
// Function to send notification email to admins - CORRECTED VERSION
...
Processed data: Row ${opsRow} of "Operations Ready" sheet
  ...
  try {
    // Parse email addresses - handle comma-separated lists
    const toEmails = EMAIL_CONFIG.to.split(',').map(email => email.trim()).filter(email => email);
    const ccEmails = EMAIL_CONFIG.cc ? EMAIL_CONFIG.cc.split(',').map(email => email.trim()).filter(email => email) : [];
    
    console.log('Sending admin notification to:', toEmails.join(', '));
    console.log('CC recipients:', ccEmails.join(', '));
    
    // The first email in the 'to' list becomes the primary recipient
    const primaryRecipient = toEmails[0];
    
    // All other 'to' emails and all 'cc' emails go in the CC field
    const allCcEmails = [...toEmails.slice(1), ...ccEmails];
    ...
    // Add CC if there are any CC recipients
    if (allCcEmails.length > 0) {
      emailOptions.cc = allCcEmails.join(',');
    }
    ...
    console.error('Email config:', EMAIL_CONFIG);
    throw error;
  }
}
...
function testEmailSending() {
  ...
  // Get current user (who is running the script)
  const currentUser = Session.getActiveUser().getEmail();
  console.log('Script is running as:', currentUser);
  console.log('Emails will be sent FROM:', currentUser);
  ...
  try {
    ...
    const testBody = `This is a test email from the Crew Portal Form system.

Configuration Details:
- Primary recipient: ${primaryRecipient}
- CC recipients: ${allCcEmails.join(', ') || 'None'}
...
If you received this email, the notification system is working correctly.`;
    ...
    const emailOptions = {
      to: primaryRecipient,
      subject: 'Test Email - Crew Portal Form System',
      body: testBody
    };
    ...
    console.log('Check the inbox of:', primaryRecipient);
    if (allCcEmails.length > 0) {
      console.log('Also check CC recipients:', allCcEmails.join(', '));
    }
  ...
  const testData = {
    postData: {
      contents: JSON.stringify({
        ...
        audioYearsExperience: '5 - 10 Years',
        workedWithPrestige: 'No',
        companiesWorkedWith: 'Test Company Inc',
        additionalComments: 'This is a test submission'
      })
    }
  };
...
function doGet(e) {
  return ContentService.createTextOutput(JSON.stringify({
    ...
    version: '1.2', // Update this when making changes
 ...
 * 
 * 1. Copy all code to Google Apps Script editor
 * 2. Save the project (File > Save or Ctrl+S)
 * 3. Run testEmailSending() to verify email configuration
 * 4. Deploy: Deploy > Manage deployments
 * 5. Click pencil icon next to active deployment
 * 6. Select "New version" in the Version dropdown
 * 7. Add description: "Fixed admin email notifications"
 * 8. Click "Deploy"
 * 9. Test with a real form submission
 * 
 * TROUBLESHOOTING:
 * - If emails aren't sending, check Executions log
 * - Verify the script is running under the correct Google account
 * - Check email quotas haven't been exceeded
 * - Run testEmailSending() to debug configuration
 */